...
Purpose: 集中管理所有文章数据（内置 + 上传），提供文章选择、添加、删除功能，实现上传文章的持久化。Output: Zustand Store + 持久化配置 + 单元测试 </objective>
...
export interface CreateArticleInput { key: string title: string description?: string content: string source: ArticleSource }
