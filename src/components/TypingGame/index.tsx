...
export function TypingGame() {
  ...
  const checkAndSubmit = useCallback(
    (value: string) => {
      ...
      if (currentChar.type === CharType.Hanzi) {
        const expectedPinyin = (
          currentChar as { quanpin: string }
        ).quanpin.toLowerCase()
  ...
  const handleCompositionEnd = useCallback(
    (e: React.CompositionEvent<HTMLInputElement>) => {
      ...
      const trimmedValue =
        maxLen > 0 ? value.toLowerCase().slice(0, maxLen) : value.toLowerCase()
  ...
  return (
    <div className={styles.container} onClick={handleContainerClick}>
      ...
      <div className={styles.header}>
        ...
        {article.description && (
          <span className={styles.author}>{article.description}</span>
        )}
      </div>
